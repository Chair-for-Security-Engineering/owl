...
static void *
my_calloc(i32 len, i32 size)
{
	void *p = calloc((u32)len, (u32)size);
	if (p == 0)
...
static void
prng_sample_secret(u64 state[4], s8 *rop, i32 len)
{
	s8 lut[] = { -1, 0, 0, 1 };
...
static void
prng_sample_error(u64 state[4], s8 *rop, i32 len)
{
	while (len--) {
		u64 r = prng_squeeze(state);
...
static void
prng_sample_unimod(u64 state[4], u64 *rop, u64 mod, i32 len)
{
 ...
 */
static void
s8_to_u64(u64 *rop, s8 const *op, u64 mod, i32 len)
{
	while (len--) {
		s64 x = *op++;
...
static void
s8_to_u64_neg(u64 *rop, s8 const *op, u64 mod, i32 len)
{
	while (len--) {
		s64 x = -*op++;
		*rop++ = (u64)x + ((u64)(x >> 63) & mod);
...
static void
normalize(u64 *rop, u64 const *op, u64 mod1, u64 mod2, i32 len)
{
	u64 half = mod1 >> 1;
...
void
moddown(Context ctx, Ciphertext *ct, i32 idx)
{
	if (ct->drop[idx] != 0)
		return;
	ct->drop[idx] = 1;
...
encrypt(Context ctx, Ciphertext *ct, s8 const *sk, u64 const *msg)
{
	...
	s8 *error = ctx.scratch;
	u64 *m = (u64 *)(error + N);
	u64 (*e)[N] = (u64 (*)[N])(m + N);
	u64 (*s)[N] = e + L;

	memcpy(m, msg, (u32)N * sizeof *m);
	...
	memset(*poly2, 0, (u32)L * (u32)N * sizeof(u64));
	memset(ct->drop, 0, (u32)L * sizeof *ct->drop);
	ct->deg = 2;
	ct->ntt = 1;
}
...
fold_swk(Context ctx, u64 fld[2][ctx.W][ctx.LK][ctx.N], u64 swk[2][ctx.W][ctx.LK][ctx.N])
{
	...
	u64 *fold = ctx.scratch;
	for (i32 i = 0; i < LK; ++i) {
		u64 mod = QP.mods[i];
		fold[i] = ring_coef_inv(prod(L, LK, 1, QP.mods, i, mod), mod);
		if (i < L)
			continue;
...
decomp_swk(Context ctx, u64 dbl[2][ctx.W2][ctx.W][ctx.R][ctx.N], u64 swk[2][ctx.W][ctx.LK][ctx.N])
{
	...
	u64 (*inv)[LKR][N] = ctx.scratch;
	f64 (*approx)[N] = (f64 (*)[N])(inv + W);
	u64 *fbcQP2E_modQP = (u64 *)(approx + W2);
	u64 (*fbcQP2E_modE)[R] = (u64 (*)[R])(fbcQP2E_modQP + LK);
	u64 (*rndQP_modE)[R] = fbcQP2E_modE + LK;
	u64 *fold = (u64 *)(rndQP_modE + W2);
	for (i32 i = 0; i < LK; ++i) {
		u64 mod = QP.mods[i];
		fbcQP2E_modQP[i] = ring_coef_inv(prod(i % W2, LK, W2, QP.mods, i, mod), mod);
	}
	for (i32 i = 0; i < R; ++i) {
		u64 mod = E.mods[i];
		for (i32 ii = 0; ii < LK; ++ii)
			fbcQP2E_modE[ii][i] = prod(ii % W2, LK, W2, QP.mods, ii, mod);
		for (i32 k2 = 0; k2 < W2; ++k2)
			rndQP_modE[k2][i] = mod - prod(k2, LK, W2, QP.mods, -1, mod);
		fold[i] = ring_coef_inv(prod(0, R, 1, E.mods, i, mod), mod);
	}
	...
	for (i32 k = 0; k < W; ++k) {
		memset(approx, 0, (u32)W2 * sizeof *approx);
		for (i32 ii = 0; ii < LK; ++ii) {
			i32 k2 = ii % W2;
	...
	for (i32 k = 0; k < ctx.W; ++k) {
		for (i32 k2 = 0; k2 < ctx.W2; ++k2) {
...
static int
test_u64(u64 const *op1, u64 const *op2, i32 len)
{
	while (len--)
		if (*op1++ != *op2++)
			return 0;
...
test(Context ctx)
{
	...
	#define TEST(s, cpy, testexpr) do {                                       \
		fputs("[+] " s "... ", stderr);                                   \
		memcpy(ct.poly, cpy.poly, 3 * (u32)L * (u32)N * sizeof *ct.poly); \
		memcpy(ct.drop, cpy.drop, (u32)L * sizeof *ct.drop);              \
		ct.deg = cpy.deg;                                                 \
		ct.ntt = cpy.ntt;                                                 \
		testexpr;                                                         \
		decrypt(ctx, m, sk, &ct);                                         \
	...
	memcpy(bwd.poly, fwd.poly, 3 * (u32)L * (u32)N * sizeof *bwd.poly);
	memcpy(bwd.drop, fwd.drop, (u32)L * sizeof *bwd.drop);
	...
	TEST("FWD to FWD (single)", fwd, swk_single(ctx, &ct, swk, 2, 1));
	TEST("FWD to BWD (single)", fwd, swk_single(ctx, &ct, swk, 2, 0));
	TEST("BWD to FWD (single)", bwd, swk_single(ctx, &ct, swk, 2, 1));
	TEST("BWD to BWD (single)", bwd, swk_single(ctx, &ct, swk, 2, 0));

	TEST("FWD to FWD (folded)", fwd, swk_folded(ctx, &ct, fld, 2, 1));
	TEST("FWD to BWD (folded)", fwd, swk_folded(ctx, &ct, fld, 2, 0));
	TEST("BWD to FWD (folded)", bwd, swk_folded(ctx, &ct, fld, 2, 1));
	TEST("BWD to BWD (folded)", bwd, swk_folded(ctx, &ct, fld, 2, 0));

	TEST("FWD to FWD (double)", fwd, swk_double(ctx, &ct, dbl, 2, 1));
	TEST("FWD to BWD (double)", fwd, swk_double(ctx, &ct, dbl, 2, 0));
	TEST("BWD to FWD (double)", bwd, swk_double(ctx, &ct, dbl, 2, 1));
	TEST("BWD to BWD (double)", bwd, swk_double(ctx, &ct, dbl, 2, 0));
	...
	i32 mu = 1, kappa = 2;
	TEST("FWD to FWD (sw2to1)", fwd, swk_switch(ctx, &ct, fld, 2, 1, mu, kappa));
	TEST("FWD to BWD (sw2to1)", fwd, swk_switch(ctx, &ct, fld, 2, 0, mu, kappa));
	TEST("BWD to FWD (sw2to1)", bwd, swk_switch(ctx, &ct, fld, 2, 1, mu, kappa));
	TEST("BWD to BWD (sw2to1)", bwd, swk_switch(ctx, &ct, fld, 2, 0, mu, kappa));
	...
	mu = 2, kappa = 3;
	TEST("FWD to FWD (sw3to2)", fwd, swk_switch(ctx, &ct, fld, 2, 1, mu, kappa));
	TEST("FWD to BWD (sw3to2)", fwd, swk_switch(ctx, &ct, fld, 2, 0, mu, kappa));
	TEST("BWD to FWD (sw3to2)", bwd, swk_switch(ctx, &ct, fld, 2, 1, mu, kappa));
	TEST("BWD to BWD (sw3to2)", bwd, swk_switch(ctx, &ct, fld, 2, 0, mu, kappa));
}
...
main(void)
{
	...
	i32 len = 3 * LKR * LKR * N;
	ctx.scratch_bytes = (u64)len * 8;
	ctx.scratch = my_calloc(len, 8);
